...
- **部品を個別に使える** — サンプリング・統計・チャンク駆動はそれぞれ単体で公開。`Simulation` はそれらを束ねる薄いラッパー。
...
`Simulation` は「サンプリング → trial → 統計」を束ねた便利ラッパーにすぎません。各段は単体でも使えます。

### ランダム生成だけ

```python
from polars_mc import sample, sample_chunks, Normal

df = sample({"w": Normal(10, 0.2), "h": Normal(5, 0.1)}, n=1000, seed=42)  # -> pl.DataFrame

# メモリ有界・再現可能なチャンク列（何億行でも一定メモリ）
for df_chunk in sample_chunks(inputs, n=10_000_000, chunk_size=1_000_000, seed=42):
    ...   # 各チャンクは独立シード
```

`sample(seed)` は、同じ seed の単一チャンク実行と同じ乱数ストリームを使います（`sample_arrays(inputs, n, rng)` はさらに低レベルで、生の NumPy 配列を返します）。

### 統計処理だけ

自前で用意した DataFrame／配列に、チャンク合成できる統計をかけます。**一発**と**逐次**の2スタイル:

```python
from polars_mc import summarize, summarize_partial, merge, Summarizer

# 一発
stats = summarize(df, {"area": ["mean", "std", ("q", 0.95)]})
stats.value("area", "mean")

# 関数型（部分集計を自分で合成）
a = summarize_partial(df1, spec)
b = summarize_partial(df2, spec)
result = merge(a, b).result()

# 逐次アキュムレータ（自前ループ）
s = Summarizer(spec)
for chunk in my_data_chunks:
    s.update(chunk)          # DataFrame でも {列: 配列} でも可
result = s.result()
```

いずれも返り値は `StatResult`（`value` / `to_dict` / `to_polars` / `summary` は `SimulationResult` と共通）。分位点は加法的でないため、内部ではリザーバ（bottom-k）サンプリングで合成しています（合成順に依存せず、予算内なら厳密）。

### チャンク駆動だけ

独立シードのチャンクループと合成、プロセス並列だけを借りて、中身は完全に自由:

```python
from polars_mc import map_reduce_chunks

def count_pass(n, rng):                      # 1チャンクの処理（中身は自由）
    area = rng.normal(10, 0.2, n) * rng.normal(5, 0.1, n)
    return int((area >= 48).sum()), n

passed, total = map_reduce_chunks(
    10_000_000, chunk_size=1_000_000, seed=0,
    map_fn=count_pass,
    reduce_fn=lambda a, b: (a[0] + b[0], a[1] + b[1]),
    backend="processes",                     # ここでもプロセス並列が使える
)
```

`chunk_sizes(n, chunk_size)` でチャンク分割だけを取り出すこともできます。
...
| [`examples/composable.py`](examples/composable.py) | 分解API。サンプリング／統計／チャンク駆動を個別に使う |
...
サンプリング sampling.py        sample / sample_chunks / sample_arrays        ← 単体公開
モデル層     （利用者のtrial関数）  1行＝1試行                               ← 利用者が書くのはここだけ
チャンク層   chunk.py           入力サンプリング → trial適用
集計層       aggregate.py       加法的モーメント合成 + リザーバ分位点
リデュース   reduce.py          DataFrame → 部分集計（chunk と summarize で共用）
統計API      summarize.py       summarize / Summarizer / merge                ← 単体公開
チャンク駆動 chunking.py        map_reduce_chunks / chunk_sizes（+バックエンド）← 単体公開
実行層       engine.py          上記を束ねる Simulation
結果層       result.py          StatResult / SimulationResult
```

`Simulation.run()` は「`map_reduce_chunks` で独立シードのチャンクを回し、各チャンクで `サンプリング → trial → reduce`、結果を merge」という合成にすぎません。
